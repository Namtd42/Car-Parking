...
public interface SQLQuery {
	...
	String ADD_PARKING = "INSERT INTO Parkinglot(parkArea, parkName, parkPlace, parkPrice) VALUES(?,?,?,?)";
	String LIST_ALL_PARKINGLOT = "SELECT * FROM Parkinglot";
	String UPDATE_PARKING = "UPDATE Parkinglot SET parkName=?, parkPlace=?, parkArea=?, parkPrice=? WHERE parkId=?";
	String DELETE_PARKING = "DELETE Parkinglot WHERE parkId=?";
	String FIND_PARKING_BYID = "SELECT * FROM Parkinglot WHERE parkId=?";
	String FIND_PARKING_BYPLACE = "SELECT * FROM Parkinglot WHERE parkPlace LIKE '%' + ? + '%'";
	String FIND_PARKING_BYNAME = "SELECT * FROM Parkinglot WHERE parkName LIKE '%' + ? + '%'";
	...
	String SEARCH_EMPLOYEE_BY_LIENCEPLACE = "SELECT c.licensePlate,c.carColor,c.carType,c.company,c.parkId, p.parkName FROM Car c JOIN Parkinglot p ON c.parkId=p.parkId WHERE licensePlate LIKE '%' + ? + '%'";
	String SEARCH_EMPLOYEE_BY_COMPANY = "SELECT c.licensePlate,c.carColor,c.carType,c.company,c.parkId, p.parkName FROM Car c JOIN Parkinglot p ON c.parkId=p.parkId WHERE company LIKE '%' + ? + '%'";
	String SEARCH_EMPLOYEE_BY_CARTYPE = "SELECT c.licensePlate,c.carColor,c.carType,c.company,c.parkId, p.parkName FROM Car c JOIN Parkinglot p ON c.parkId=p.parkId WHERE carType LIKE '%' + ? + '%'";
	...
	String ADD_TICKET = "INSERT INTO Ticket VALUES(?,?,?,?)";
